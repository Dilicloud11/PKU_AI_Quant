...
def build():
    ...
    toc = [
        ...
        ("2.3 跨策略风险调整绩效对比", "5", 1),
        ("2.4 重点策略深度剖析：ETF 双重动量轮动", "5", 1),
        ("2.5 策略间的关联性与互补性", "8", 1),
        ("2.6 多策略量化交易系统的构建思路", "8", 1),
        ("三、机器学习在量化交易中的应用总结", "9", 0),
        ("3.1 数据预处理与特征工程", "9", 1),
        ("3.2 模型选择、训练与评估优化", "9", 1),
        ("3.3 机器学习的优势、局限与未来趋势", "10", 1),
        ("四、结论与展望", "11", 0),
        ("附录：改进建议", "12", 0),
    ]
    for t, pg, lv in toc:
    ...
    # ---- 2.4 ETF 轮动专项图表分析（重点策略，多维图表）----
    etf_m = pd.read_csv(os.path.join(BASE, "etf_metrics.csv"), encoding="utf-8-sig")
    def em(k, c):
        return etf_m[etf_m["方案"] == k][c].iloc[0]
    h2(doc, "2.4 重点策略深度剖析：ETF 双重动量轮动")
    body(doc,
        "鉴于 ETF 双重动量轮动是本工作坊综合表现最优的策略，本节对其做多维度的图表化剖析。图 2 为资产曲线对比，"
        "改进版 v1.1（双周调仓）以总收益 " + P(em("v1.1双周", "总收益")) + "、年化 " + P(em("v1.1双周", "年化")) +
        " 显著跑赢原版 v1.0（周度）、等权持有与沪深300。")
    pic(doc, "etf_A_equity.png", 6.3)
    caption(doc, "图 2　ETF 轮动策略资产曲线对比（策略 vs 基准）")
    body(doc,
        "图 3 的回撤曲线显示，v1.1 最大回撤仅 " + P(em("v1.1双周", "最大回撤")) + "，明显浅于 v1.0 与等权持有，"
        "风险控制更优。")
    pic(doc, "etf_B_drawdown.png", 6.3)
    caption(doc, "图 3　ETF 轮动策略回撤曲线（风险特征）")
    body(doc,
        "图 4 的月度收益热力图（红涨绿跌）展示了收益的时间分布：正收益月份（红）明显多于负收益月份（绿），"
        "且在 2020、2025 等年份的多个月份取得两位数正收益，收益来源在时间上较为分散、并非依赖个别月份。")
    pic(doc, "etf_C_monthly_heatmap.png", 6.3)
    caption(doc, "图 4　ETF 轮动 v1.1 月度收益热力图（时间分布）")
    body(doc,
        "图 5 的日收益分布直方图显示，v1.1 的收益分布相对沪深300 更向右偏移（日均收益为正）、且尖峰特征明显，"
        "反映其在控制单日波动的同时积累正向收益。")
    pic(doc, "etf_D_return_hist.png", 6.3)
    caption(doc, "图 5　ETF 轮动 v1.1 日收益分布直方图（收益特征）")
    body(doc,
        "图 6 的滚动 1 年夏普比率考察策略稳定性：v1.1 的滚动夏普在多数时间位于零轴以上、且频繁高于 1，"
        "整体稳定性优于等权持有，说明其超额并非来自某一段行情的偶然。")
    pic(doc, "etf_E_rolling_sharpe.png", 6.3)
    caption(doc, "图 6　ETF 轮动 v1.1 滚动 1 年夏普比率（稳定性）")
    body(doc,
        "图 7 的月度持仓轮动图直观呈现了策略的“信号验证”：在不同市场环境下，策略确实在宽基、行业与黄金等大类间"
        "动态切换——下跌与震荡期黄金（避险）权重上升，上涨期则向创业板、券商等高弹性板块倾斜，轮动逻辑符合预期。")
    pic(doc, "etf_F_holdings.png", 6.3)
    caption(doc, "图 7　ETF 轮动 v1.1 月度持仓轮动图（选中标的与权重，信号验证）")

    h2(doc, "2.5 策略间的关联性与互补性")
    body(doc,
        "各类策略并非彼此孤立，而是在“进攻—均衡—防御”的谱系上相互补位，这正是构建组合的基础。任务七的三个策略"
        "构成了一个天然的互补三角，其在不同牛熊阶段的表现对比如图 8 所示。")
    pic(doc, "task7_compare.png", 6.4)
    caption(doc, "图 8　三类主力策略的跨牛熊表现对比（净值、回撤、分阶段收益与风险-收益散点）")
    body(doc,
